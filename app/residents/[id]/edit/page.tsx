...
export default function EditResident(){
 ...
 const [f,setF]=useState({building:'',apartment:'',first_name:'',last_name:'',phone:''}); const [msg,setMsg]=useState(''); const [busy,setBusy]=useState(true);
 useEffect(()=>{(async()=>{const {data,error}=await supabase.from('residents').select('building,apartment,first_name,last_name,phone').eq('id',id).single(); if(error) setMsg('تعذر تحميل بيانات الجار.'); else setF({building:data.building||'',apartment:data.apartment||'',first_name:data.first_name||'',last_name:data.last_name||'',phone:data.phone||''}); setBusy(false)})()},[id]);
 async function save(e:any){e.preventDefault();setBusy(true);const full_name=`${f.first_name} ${f.last_name}`.trim();const {error}=await supabase.from('residents').update({...f,full_name,updated_at:new Date().toISOString()}).eq('id',id);setMsg(error?`تعذر التعديل: ${error.message}`:'تم تعديل بيانات الجار بنجاح.');setBusy(false);if(!error)setTimeout(()=>router.push(`/residents/${id}`),500)}
