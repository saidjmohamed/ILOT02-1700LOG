...
export default function Home(){
 ...
 const [tx,setTx]=useState<Tx[]>([]); const [loading,setLoading]=useState(true); const [error,setError]=useState(''); const [canManage,setCanManage]=useState(false);
 useEffect(()=>{ (async()=>{ setLoading(true); const {data:{user}}=await supabase.auth.getUser(); if(user){const {data:p}=await supabase.from('profiles').select('role').eq('id',user.id).maybeSingle();setCanManage(['super_admin','admin','treasurer'].includes(p?.role||''));}
   const [{data:sum,error:se},{data:rows,error:re},{data:contribs}]=await Promise.all([
   ...
   supabase.from('contributions').select('id,amount,contribution_date,residents(full_name),treasuries(name)').order('contribution_date',{ascending:false}).limit(8)
 ]); if(se && se.code!=='PGRST116') setError('تعذر تحميل الملخص المالي.'); if(re) setError('تعذر تحميل العمليات.'); if(sum) setSummary(sum as Summary);
   const expenseRows=(rows||[]).map((r:any)=>({...r,kind:'expense' as const})); const contributionRows=(contribs||[]).map((r:any)=>({id:r.id,kind:'income' as const,title:`مساهمة ${r.residents?.full_name||'جار'}`,amount:Number(r.amount),transaction_date:r.contribution_date,description:r.treasuries?.name?`خزينة: ${r.treasuries.name}`:null})); setTx([...expenseRows,...contributionRows].sort((a,b)=>String(b.transaction_date).localeCompare(String(a.transaction_date))).slice(0,8)); setLoading(false); })(); },[]);
 return <div className="app"><header className="topbar"><div className="brand"><div className="brandMark">🏘️</div><div><h1>خزينة الحي</h1><p>حي 1700 مسكن بوعنقود — البليدة إيلو 02</p></div></div><nav className="nav"><Link href="/">الرئيسية</Link><Link href="/announcements">📢 الإعلانات</Link><Link href="/residents">الجيران</Link><Link href="/treasuries">الخزائن</Link><Link href="/contributions">المساهمات</Link><Link href="/expenses">المصاريف</Link><Link href="/reports">التقارير</Link></nav></header><main className="wrap">
 ...
 <section className="stats"><Stat label="عدد الجيران" value={loading?'—':String(summary.active_residents)} /><Stat label="إجمالي الأموال" value={loading?'—':money(summary.total_income)} /><Stat label="إجمالي المصاريف" value={loading?'—':money(summary.total_expense)} /><Stat label="الرصيد الحالي" value={loading?'—':money(summary.balance)} /></section>
 ...
 <div className="card"><div className="sectionTitle"><h3>اختصارات</h3></div><div className="quick"><Link href="/announcements">📢<br/>إعلانات الحي</Link><Link href="/residents">👥<br/>الجيران</Link>{canManage&&<><Link href="/residents/new">➕<br/>إضافة جار</Link><Link href="/treasuries/new">🏦<br/>خزينة جديدة</Link><Link href="/contributions">💰<br/>المساهمات</Link><Link href="/expenses">🧾<br/>المصاريف</Link></>}</div></div></aside></section>
