...
export default function Residents(){
 ...
 async function load(){setLoading(true);let query=supabase.from('public_residents').select('id,building,apartment,full_name,is_active').order('building').order('apartment'); if(building) query=query.eq('building',building); const {data}=await query; setRows(data||[]); setLoading(false)}
